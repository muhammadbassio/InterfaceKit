...
open class IKAnimatableButton: UIButton {
  ...
  public func path(for layerIndex:Int, state:Int) -> CGPath? {
    if let layerPaths = self.paths[layerIndex] { return layerPaths[state] }
    return nil
  }
  
  public func addPath(path:CGPath, for layerIndex:Int, and state:Int) { self.paths[layerIndex] = [state : path] }
